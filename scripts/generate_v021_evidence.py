...
from sentinel_edge.domain.models import BenchmarkVariant, ClaimClass, HostQualificationReport, HazardKind, SensorKind, CapabilityState
...
def _copy_evidence(name: str, source: Path) -> None:
    shutil.copy2(source, ROOT / "evidence" / f"v0.21.0-{name}")
...
def build() -> dict:
    ...
    target_manifest_path = ROOT / "fixtures/release/arm64-target-wheel-acquisition-v0.21.0.json"
    ...
    config_path = ROOT / "fixtures/configuration/default-v0.21.0.yaml"
    ...
    identity_specs = (
        ...
        FileIdentitySpec(name="configuration", path="fixtures/configuration/default-v0.21.0.yaml", sha256=sha256_file(config_path)),
    ...
    _write_json(ROOT / "fixtures/benchmark/v0.21.0-identity-spec.json", identity_spec_payload)
    ...
    plan = BenchmarkAnalysisPlan(
        plan_id="sentinel-edge-v0.21.0-confirmatory-development-envelope",
    ...
    _write_public(evidence / "v0.21.0-benchmark-plan-public.pem", plan_key)
    _write_json(ROOT / "fixtures/benchmark/v0.21.0-signed-analysis-plan.json", signed_plan.model_dump(mode="json"))
        ...
        for variant, metrics in zip(BenchmarkVariant, values):
            samples.append(BenchmarkSample(
                ...
                energy=EnergyMeasurement(
                    ...
                    quality_target_id="v0.21.0-development-quality-guardrail",
    ...
    execution = BenchmarkExecutionRecord(
        run_id="v0.21.0-development-analysis-rehearsal",
    ...
    _write_json(ROOT / "fixtures/benchmark/v0.21.0-development-execution.json", execution.model_dump(mode="json"))
    ...
    proxy_energy = evaluate_energy_comparison(tuple(
        EnergyMeasurement(
            ...
            method="deterministic-work-proxy", quality_target_id="v0.21.0-development-quality-guardrail",
    ...
    physical_fixture = evaluate_energy_comparison(tuple(
        EnergyMeasurement(
            ...
            method="external-inline-meter-fixture", quality_target_id="v0.21.0-development-quality-guardrail",
    ...
    issue_registry = RuntimeKnownIssueRegistry(
        ...
        issues=(RuntimeKnownIssue(
            ...
            match=RuntimeIssueMatch(
                runtime_name="sentinel-edge", runtime_version="0.21.0", provider="CPUExecutionProvider",
                architecture=platform.machine(), graph_sha256="f" * 64,
            ),
            default_disposition=RuntimeIssueDisposition.DENY,
            evidence_refs=("tests/test_runtime_known_issues_v021.py",),
    ...
    _write_public(evidence / "v0.21.0-runtime-issues-public.pem", issue_key)
    _write_json(ROOT / "fixtures/runtime/runtime-known-issues-v0.21.0.json", signed_issue_registry.model_dump(mode="json"))
    ...
    _write_public(evidence / "v0.21.0-signal-chain-public.pem", signal_key)
    ...
    # v0.21 qualification vocabulary, evidence windows, platform envelope and runtime KAT.
    benchmark_host_observation = observe_benchmark_host(provider_threads=1)
    _write_json(ROOT / "qualification/benchmark-host-observation.json", benchmark_host_observation)
    host_path = write_host_trust_report(ROOT / "host-trust-report.json")
    host_trust_payload = json.loads(host_path.read_text(encoding="utf-8"))
    platform_envelope = build_platform_runtime_envelope(
        envelope_id="development-platform-envelope-v0.21.0",
        benchmark_host_report=benchmark_host_observation,
        host_trust_report=host_trust_payload,
        runtime_identities={"sentinel-edge": "0.21.0", "python": platform.python_version(), "provider": "fixture-provider"},
        driver_identities={"imu": "development-driver-v1", "camera": "development-camera-driver-v1"},
        hardening_identities={"kernel_lockdown": str(host_trust_payload.get("kernel", {}).get("lockdown")), "lsm": str(host_trust_payload.get("kernel", {}).get("lsm"))},
        observed_at=datetime(2026, 8, 3, 9, 0, tzinfo=timezone.utc),
        expires_at=datetime(2026, 9, 2, 9, 0, tzinfo=timezone.utc),
        source_class="fixture",
    )
    platform_envelope_path = ROOT / "fixtures/qualification/platform-runtime-envelope-v0.21.0.json"
    _write_json(platform_envelope_path, platform_envelope.model_dump(mode="json", by_alias=True))
    platform_expected = {
        "target_family": "raspberry-pi-5", "machine": "aarch64", "require_observed_source": True,
        "runtime_identities": {"sentinel-edge": "0.21.0", "provider": "CPUExecutionProvider"},
        "driver_identities": {"imu": "qualified-target-driver"}, "hardening_identities": {"kernel_lockdown": "integrity"},
        "claimed_cpu_features": ["asimd"],
    }
    _write_json(ROOT / "fixtures/qualification/platform-runtime-envelope-expected-v0.21.0.json", platform_expected)
    platform_envelope_report = evaluate_platform_runtime_envelope(
        platform_envelope, expected=platform_expected, now=datetime(2026, 8, 3, 9, 5, tzinfo=timezone.utc)
    )
    _write_json(ROOT / "qualification/platform-runtime-envelope.json", platform_envelope_report)

    window_specs = [
        ("platform", QualificationDomain.PLATFORM, platform_envelope_report["report_digest"], CapabilityState.TESTED),
        ("model", QualificationDomain.MODEL, model_package_report["report_digest"], CapabilityState.TESTED),
        ("calibration", QualificationDomain.CALIBRATION, commissioning_report["report_digest"], CapabilityState.TESTED),
        ("security", QualificationDomain.SECURITY, issue_report["report_digest"], CapabilityState.TESTED),
        ("source", QualificationDomain.SOURCE, imu_signal_report["report_digest"], CapabilityState.TESTED),
    ]
    windows = []
    evaluations = []
    for name, domain, digest, state in window_specs:
        window = QualificationEvidenceWindow(
            qualification_id=f"v0.21.0-{name}", domain=domain, subject_id=f"development-{name}", state=state,
            evidence_sha256=digest, valid_from=datetime(2026, 8, 3, 9, 0, tzinfo=timezone.utc),
            valid_until=datetime(2026, 9, 2, 9, 0, tzinfo=timezone.utc), provenance=EvidenceProvenance.FIXTURE,
            limitations=("development_fixture_not_target_qualification",),
        )
        windows.append(window.model_dump(mode="json", by_alias=True))
        evaluations.append(evaluate_evidence_window(window, now=datetime(2026, 8, 3, 9, 5, tzinfo=timezone.utc)))
    claim_decision = enforce_claim_ceiling(
        QualificationClaim(capability_id="sentinel-edge-target-runtime", requested_state=CapabilityState.TARGET_QUALIFIED, statement="Target runtime qualified"),
        evaluations,
    )
    lifecycle_base = {
        "schema": "sentinel-edge-qualification-lifecycle-report/1.0",
        "vocabulary": qualification_vocabulary_report(), "windows": windows, "evaluations": evaluations,
        "claim_decision": claim_decision, "target_claim_allowed": claim_decision["allowed"],
        "expired_evidence_blocks_claims": True, "reconstructed_evidence_cannot_invent_qualification": True,
        "release_eligible": False,
    }
    lifecycle_report = {**lifecycle_base, "report_digest": sha256_bytes(canonical_json_bytes(lifecycle_base))}
    _write_json(ROOT / "qualification/qualification-lifecycle.json", lifecycle_report)

    class _FixtureRuntimeSession:
        def run(self, inputs: dict[str, Any]) -> dict[str, Any]:
            value = float(inputs["value"]); branch = "positive" if value >= 0 else "negative"
            return {"score": 1.0 if branch == "positive" else 0.0, "branch": branch}
    kat_suite = KnownAnswerSuite(
        suite_id="seismic-development-known-answer-v0.21.0", model_graph_sha256=model_package_report["graph_capability_sha256"],
        runtime_profile_sha256=runtime_digest, required_branches=("positive", "negative"),
        cases=(
    ...
    _write_json(ROOT / "fixtures/runtime/seismic-known-answer-v0.21.0.json", kat_suite.model_dump(mode="json"))
    kat_report = run_known_answer_suite(kat_suite, _FixtureRuntimeSession, target_host_qualified=False, target_runtime_qualified=False)
    _write_json(ROOT / "qualification/runtime-known-answer.json", kat_report)
    ...
    attestations = [
        build_attestation(builder_id="sentinel-local-builder-a", host_fingerprint=host_fingerprint, environment_digest=sha256_bytes(b"v021-builder-a"), source_digest=repro["source_members_digest"], artifact_digest=repro["artifact_classes"][0]["build_a"]["sha256"], private_key=builder_a),
        build_attestation(builder_id="sentinel-local-builder-b", host_fingerprint=host_fingerprint, environment_digest=sha256_bytes(b"v021-builder-b"), source_digest=repro["source_members_digest"], artifact_digest=repro["artifact_classes"][0]["build_a"]["sha256"], private_key=builder_b),
    ]
    ...
    _write_public(evidence / "v0.21.0-builder-a-public.pem", builder_a)
    _write_public(evidence / "v0.21.0-builder-b-public.pem", builder_b)

    release_public = load_pem_public_key((evidence / "v0.21.0-release-public.pem").read_bytes())
    ...
    copies = {
        ...
        "benchmark-host-observation.json": ROOT / "qualification/benchmark-host-observation.json",
        "platform-runtime-envelope.json": ROOT / "qualification/platform-runtime-envelope.json",
        "qualification-lifecycle.json": ROOT / "qualification/qualification-lifecycle.json",
        "runtime-known-answer.json": ROOT / "qualification/runtime-known-answer.json",
    }
    for name, source in copies.items():
        _copy_evidence(name, source)

    base = {
        "schema": "sentinel-edge-v021-evidence/1.0",
        ...
        "platform_runtime_envelope": {"path": "qualification/platform-runtime-envelope.json", "sha256": sha256_file(ROOT / "qualification/platform-runtime-envelope.json"), "target_qualified": platform_envelope_report["target_qualified"], "failures": platform_envelope_report["failures"]},
        "qualification_lifecycle": {"path": "qualification/qualification-lifecycle.json", "sha256": sha256_file(ROOT / "qualification/qualification-lifecycle.json"), "target_claim_allowed": lifecycle_report["target_claim_allowed"]},
        "runtime_known_answer": {"path": "qualification/runtime-known-answer.json", "sha256": sha256_file(ROOT / "qualification/runtime-known-answer.json"), "development_passed": kat_report["development_passed"], "target_qualified": kat_report["target_qualified"]},
        "contract_truth": {"implemented": 254, "confirmed": 489, "verified": 0, "h0_implemented": 97, "h0_open": 143},
        "limitations": [
            ...
            "The platform envelope, evidence windows and runtime known-answer suite are fixture/development evidence and remain non-target.",
    ...
    _write_json(evidence / "v0.21.0-target-benchmark-governance.json", receipt)
    return receipt
