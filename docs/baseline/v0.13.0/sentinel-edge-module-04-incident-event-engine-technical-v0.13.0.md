# Sentinel Edge — Module 04 Incident & Event Engine Technical Architecture & Specifications

**Document ID:** SE-INC-TAS-013  
**Version:** 0.13.0  
**Date:** 2026-07-26  
**Status:** Independently deployable/testable module architecture  
**Paired functional document:** SE-INC-FPD-013  
**System integration contract:** SE-SYS-TAS-013

## 1. Architectural decision

Module 04 is a **hexagonal module/bounded context** with a composition root, owned state, public inbound/outbound ports and replaceable adapters. It may run in-process in the compact profile or as an isolated process in the service profile. The same contracts and tests apply to both profiles.
...
├── pyproject.toml
├── README.md
├── src/sentinel_incident_event_engine/
...
| Port | Responsibility |
|---|---|
| IncidentInputPort | accept inference/analysis/source-health envelopes |
| IncidentCommandPort | accept validated idempotent commands |
| IncidentRepositoryPort | own event journal/snapshots/projections |
| OutboxPort | persist/dispatch side-effect intents |
| ProjectionEventPort | publish authoritative projections |
...
incident.db containing inbox, append-only incident journal, snapshots/projections, review log and transactional outbox; this is the only incident authority.
...
### Incident consistency model

Commands and inputs enter an idempotent inbox. A transaction appends one or more domain events, updates the materialized projection/snapshot and inserts outbox intents. Optimistic concurrency uses expected incident version. Rebuild from the journal must reproduce the same semantic projection under the declared determinism profile.

## 6. Plugin architecture

| Plugin type | Responsibility | Entry-point group |
|---|---|---|
| HazardPolicyPack | Defines one hazard vocabulary, state machine, coverage gates and transition rules. | `sentinel.incident_event_engine.plugins` |
| IncidentMatcherPlugin | Proposes candidate deduplication/link/merge matches without committing them. | `sentinel.incident_event_engine.plugins` |
| CorroborationPolicyPlugin | Evaluates independence, support and contradiction under explicit rules. | `sentinel.incident_event_engine.plugins` |
| ReviewPolicyPlugin | Defines acknowledgement, snooze, escalation and resolution constraints. | `sentinel.incident_event_engine.plugins` |
| ProjectionBuilderPlugin | Builds additional read projections from the authoritative event journal. | `sentinel.incident_event_engine.plugins` |
| EffectAdapterPlugin | Dispatches outbox intents; it cannot alter historical incident truth. | `sentinel.incident_event_engine.plugins` |

### Manifest

```yaml
plugin_id: sentinel.incident.hazard.example
...
|---|---|
| 1 | table-driven transition tests for each hazard policy pack |
| 2 | Hypothesis state-machine tests for create/update/merge/split/resolve/reopen sequences |
| 3 | invariant tests for coverage-gated negative evidence and external-only verification prohibition |
| 4 | crash/retry tests for inbox idempotency, transactional outbox and snapshot recovery |
| 5 | consumer/provider contracts with Analyzer, Runtime, API and projection subscribers |
| 6 | whole-scenario semantic determinism and After-Event Review reconciliation tests |
...
```bash
make test-incident-event-engine
make component-incident-event-engine        # starts only this module with fake ports
make conformance-incident-event-engine      # runs every installed/selected plugin
make contracts-incident-event-engine        # schemas, examples, compatibility and generated code
```
