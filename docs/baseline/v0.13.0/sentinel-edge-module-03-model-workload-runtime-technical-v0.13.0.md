# Sentinel Edge — Module 03 Model & Workload Runtime Technical Architecture & Specifications

**Document ID:** SE-RUN-TAS-013  
**Version:** 0.13.0  
**Date:** 2026-07-26  
**Status:** Independently deployable/testable module architecture  
**Paired functional document:** SE-RUN-FPD-013  
**System integration contract:** SE-SYS-TAS-013

## 1. Architectural decision

Module 03 is a **hexagonal module/bounded context** with a composition root, owned state, public inbound/outbound ports and replaceable adapters. It may run in-process in the compact profile or as an isolated process in the service profile. The same contracts and tests apply to both profiles.
...
├── pyproject.toml
├── README.md
├── src/sentinel_model_workload_runtime/
...
| Port | Responsibility |
|---|---|
| JobSubmissionPort | accept validated ModelJobRequest |
| ModelRegistryPort | read signed model/workload profiles |
| WorkerPort | execute isolated jobs |
| ResourceProbePort | read pressure/thermal/power |
| ResultEventPort | emit ModelResult/HazardInference |
...
runtime.db for registry/qualification/job telemetry plus read-only model directory; no arbitrary model upload or URL.
...
The reference policy is fixed criticality plus earliest deadline first within a tier, with aging, Tier A reserve, interference inflation, memory-pressure gates and validated fallbacks. `SCHED_OTHER` is the release baseline. The scheduler controls admission and placement; it never claims arbitrary interruption of native kernels.

## 6. Plugin architecture

| Plugin type | Responsibility | Entry-point group |
|---|---|---|
| WorkloadPlugin | Defines preprocess/execute/postprocess and a bounded cancellation contract. | `sentinel.model_workload_runtime.plugins` |
| InferenceBackendPlugin | Executes an approved model format/provider such as ONNX Runtime CPU EP. | `sentinel.model_workload_runtime.plugins` |
| SchedulerPolicyPlugin | Ranks/admit jobs; only one signed release policy is active. | `sentinel.model_workload_runtime.plugins` |
| ResourceProbePlugin | Reports CPU, memory PSI, thermal, power and storage pressure. | `sentinel.model_workload_runtime.plugins` |
| PreprocessorPlugin | Transforms canonical inputs into model tensors under a versioned recipe. | `sentinel.model_workload_runtime.plugins` |
| PostprocessorPlugin | Calibrates scores, thresholds, ambiguity margins and structured outputs. | `sentinel.model_workload_runtime.plugins` |

### Manifest

```yaml
plugin_id: sentinel.runtime.workload.example
...
|---|---|
| 1 | pure scheduler unit tests with virtual clock and deterministic workload costs |
| 2 | property/stateful tests for starvation, cancellation, retries, model residency and overload transitions |
| 3 | known-answer and event-level quality tests for every model profile |
| 4 | provider/backend conformance tests with malformed model and external-data rejection corpus |
| 5 | interference and memory-pressure tests on target Arm hardware |
| 6 | contract tests for job requests/results with Collector, Analyzer and Incident Engine |
...
```bash
make test-model-workload-runtime
make component-model-workload-runtime        # starts only this module with fake ports
make conformance-model-workload-runtime      # runs every installed/selected plugin
make contracts-model-workload-runtime        # schemas, examples, compatibility and generated code
```
