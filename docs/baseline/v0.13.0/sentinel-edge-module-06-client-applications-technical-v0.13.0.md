# Sentinel Edge — Module 06 Client Applications — Web + Mobile Technical Architecture & Specifications

**Document ID:** SE-CLI-TAS-013  
**Version:** 0.13.0  
**Date:** 2026-07-26  
**Status:** Independently deployable/testable module architecture  
**Paired functional document:** SE-CLI-FPD-013  
**System integration contract:** SE-SYS-TAS-013

## 1. Architectural decision

Module 06 is a **hexagonal module/bounded context** with a composition root, owned state, public inbound/outbound ports and replaceable adapters. It may run in-process in the compact profile or as an isolated process in the service profile. The same contracts and tests apply to both profiles.
...
├── pyproject.toml
├── README.md
├── src/sentinel_client_applications/
...
| Port | Responsibility |
|---|---|
| ApiClientPort | generated REST client |
| ProjectionClientPort | SSE/WebSocket resume |
| OfflineCommandPort | queue/retry/conflict |
| LocalStorePort | cache and secure preferences |
| DeviceCapabilityPort | capture/notification/share |
...
client-local encrypted cache/queue; server projections remain authoritative and cache migrations are independently versioned.
...
The client component contains separate `web`, `mobile-shell` and `shared-domain` packages. The shared package contains generated API types, vocabulary, formatting, trust/coverage presentation rules and offline command state. It contains no browser- or mobile-specific UI framework code.

## 6. Plugin architecture

| Plugin type | Responsibility | Entry-point group |
|---|---|---|
| EvidenceRendererPlugin | Renders an allowlisted evidence modality/type using sanitized data. | `sentinel.client_applications.plugins` |
| CaptureProviderPlugin | Captures/uploads one supported mobile/web modality under privacy policy. | `sentinel.client_applications.plugins` |
| NotificationAdapterPlugin | Maps server intents to a local notification channel. | `sentinel.client_applications.plugins` |
| LocalizationPackPlugin | Provides versioned translations and terminology tests. | `sentinel.client_applications.plugins` |
| ReadOnlyPanelPlugin | Adds a bounded view over published API data; no arbitrary remote code. | `sentinel.client_applications.plugins` |
| OfflineStorePlugin | Implements encrypted/local cache storage behind one interface. | `sentinel.client_applications.plugins` |

### Manifest

```yaml
plugin_id: sentinel.client.renderer.example
...
|---|---|
| 1 | shared domain/type/terminology tests executed for both web and mobile packages |
| 2 | component tests with generated API mocks and deterministic projection transcripts |
| 3 | consumer contract tests against Module 05 provider contracts |
| 4 | offline queue state-machine tests for retry, duplicate, conflict and expiry |
| 5 | accessibility automation plus manual keyboard/screen-reader/reduced-motion evidence |
| 6 | end-to-end browser/mobile-shell tests against the composed solution scenario |
...
```bash
make test-client-applications
make component-client-applications        # starts only this module with fake ports
make conformance-client-applications      # runs every installed/selected plugin
make contracts-client-applications        # schemas, examples, compatibility and generated code
```
