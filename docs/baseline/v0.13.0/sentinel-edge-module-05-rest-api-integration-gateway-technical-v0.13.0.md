# Sentinel Edge — Module 05 REST API & Integration Gateway Technical Architecture & Specifications

**Document ID:** SE-API-TAS-013  
**Version:** 0.13.0  
**Date:** 2026-07-26  
**Status:** Independently deployable/testable module architecture  
**Paired functional document:** SE-API-FPD-013  
**System integration contract:** SE-SYS-TAS-013

## 1. Architectural decision

Module 05 is a **hexagonal module/bounded context** with a composition root, owned state, public inbound/outbound ports and replaceable adapters. It may run in-process in the compact profile or as an isolated process in the service profile. The same contracts and tests apply to both profiles.
...
├── pyproject.toml
├── README.md
├── src/sentinel_rest_api_integration_gateway/
...
| Port | Responsibility |
|---|---|
| HTTPQueryPort | serve projections and diagnostics |
| HTTPCommandPort | validate/map commands |
| IncidentClientPort | send commands/read results from Incident Engine |
| ProjectionSubscriptionPort | consume/resume projections |
| IdentityPolicyPort | authenticate/authorize/filter |
...
api.db only for sessions, rate limits, projection cursors and audit context; incident projections are consumed, not reconstructed from incident tables.
...
The implementation emits OpenAPI 3.1.2 for current FastAPI/client-tool compatibility, while tracking OpenAPI 3.2.0 as the latest published specification. API evolution uses additive changes within a major version, explicit deprecation, compatibility tests and a bounded N/N-1 support window.

## 6. Plugin architecture

| Plugin type | Responsibility | Entry-point group |
|---|---|---|
| AuthProviderPlugin | Authenticates a supported local or deployment identity method. | `sentinel.rest_api_integration_gateway.plugins` |
| AuthorizationPolicyPlugin | Evaluates scoped actions and field/precision filtering. | `sentinel.rest_api_integration_gateway.plugins` |
| ProjectionTransportPlugin | Implements SSE/WebSocket/local stream delivery with resume cursors. | `sentinel.rest_api_integration_gateway.plugins` |
| ExportFormatPlugin | Builds a derived authorized export from immutable references. | `sentinel.rest_api_integration_gateway.plugins` |
| ReadResourcePlugin | Adds a read-only bounded resource; no plugin receives incident repositories. | `sentinel.rest_api_integration_gateway.plugins` |
| RateLimitPolicyPlugin | Applies route/identity/resource-specific limits. | `sentinel.rest_api_integration_gateway.plugins` |

### Manifest

```yaml
plugin_id: sentinel.api.export.example
...
|---|---|
| 1 | OpenAPI schema/lint and generated-client compilation tests |
| 2 | consumer-driven contract tests for web/mobile and third-party integrations |
| 3 | authorization matrix and privacy-field filtering tests |
| 4 | idempotency, ETag, concurrency-conflict, pagination and cursor-resume tests |
| 5 | black-box component tests with a fake Incident Engine port |
| 6 | pairwise and vertical-slice tests with the real Incident Engine and client shells |
...
```bash
make test-rest-api-integration-gateway
make component-rest-api-integration-gateway        # starts only this module with fake ports
make conformance-rest-api-integration-gateway      # runs every installed/selected plugin
make contracts-rest-api-integration-gateway        # schemas, examples, compatibility and generated code
```
